...
def _build_schema(defaults: dict | None = None) -> vol.Schema:
    ...
    if defaults.get(CONF_LIGHT_ENTITY):
        schema[
            vol.Optional(
                CONF_LIGHT_ENTITY,
                default=defaults[CONF_LIGHT_ENTITY],
            )
        ] = selector.EntitySelector(
            selector.EntitySelectorConfig(domain="sensor")
        )
    ...
    if defaults.get(CONF_TEMP_ENTITY):
        schema[
            vol.Optional(
                CONF_TEMP_ENTITY,
                default=defaults[CONF_TEMP_ENTITY],
            )
        ] = selector.EntitySelector(
            selector.EntitySelectorConfig(domain="sensor")
        )
    else:
        schema[vol.Optional(CONF_TEMP_ENTITY)] = selector.EntitySelector(
            selector.EntitySelectorConfig(domain="sensor")
        )
        ...
        if user_input is not None:
            ...
            if not plant_name:
                ...
                if not user_input.get(CONF_LIGHT_ENTITY):
                    user_input.pop(CONF_LIGHT_ENTITY, None)

                if not user_input.get(CONF_TEMP_ENTITY):
                    user_input.pop(CONF_TEMP_ENTITY, None)

                if not user_input.get(CONF_SPECIES):
                    user_input.pop(CONF_SPECIES, None)

                if not user_input.get(CONF_IMAGE_URL):
                    user_input.pop(CONF_IMAGE_URL, None)
